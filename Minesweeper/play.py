...
def solveProbabilityOnes():
    ...
    cc = 0
    ab = 0
    for aa in explorableCells:
        ab += 1
        for bb in aa:
            cc += 1
    dd = cc*(1)
    with alive_bar(ab) as bar1:
        for cellGroup in explorableCells:
            ...
            for coordinate in cellGroup:
                ...
                cellsAndSurroundingDict[coordinate] = nearbyIncompleteCells(coordinate[0], coordinate[1])
            ...
            def tryPossibiility(attempt, index, changeMine, incompletedCellDict, minesSum):
                ...
                if index == len(possibilities)-1:
                    return
                tryPossibiility(attempt.copy(), index+1, False, incompletedCellDict.copy(), minesSum)
                tryPossibiility(attempt.copy(), index+1, True, incompletedCellDict.copy(), minesSum)
...
while movesLeft > 0:
    ...
    if len(clickLeftList) == 0 and len(clickRightList) == 0:
        ...
        if cellsProbList != []:
            ...
            from boardConverter import unsolved
            clickLeftList.append(unsolved[0])
